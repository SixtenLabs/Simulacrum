...
namespace SixtenLabs.Simulacrum
{
	...
	public abstract class Simulator : ISimulator
  {
    public Simulator(IComponentManagerFactory componentManagerFactory)
    {
      ...
      ComponentManager = ComponentManagerFactory.CreateComponentManager(RegisteredComponentTypes);
    }

    protected abstract void SetupProperties();

    /// <summary>
    /// All component types this simulator supports must be registered.
    /// </summary>
    ...
    protected IList<Type> RegisteredComponentTypes { get; } = new List<Type>();

    private IComponentManagerFactory ComponentManagerFactory { get; }
  }
}
